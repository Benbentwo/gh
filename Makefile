...
GO_DEPENDENCIES := $(call rwildcard,pkg/,*.go) $(call rwildcard,cmd/gh/,*.go)
...
# support for building a covered gh binary (one with the coverage instrumentation compiled in). The `build-covered`
# target also builds the covered binary explicitly
COVERED_MAIN_SRC_FILE=./cmd/gh
...
.PHONY: build
build: $(GO_DEPENDENCIES) ## Build gh binary for current OS
...
.PHONY: build-covered
build-covered: $(GO_DEPENDENCIES) ## Build gh binary for current OS with coverage instrumentation to build/$(NAME).covered
...
testbin: get-test-deps make-reports-dir
	CGO_ENABLED=$(CGO_ENABLED) $(GOTEST) -c github.com/Benbentwo/gh/pkg/cmd -o build/gh-test

#testbin-gits: get-test-deps make-reports-dir
#	CGO_ENABLED=$(CGO_ENABLED) $(GOTEST) -c github.com/Benbentwo/gh/pkg/gits -o build/gh-test-gits

debugtest1: testbin
	cd pkg/gh/cmd && dlv --listen=:2345 --headless=true --api-version=2 exec ../../../build/gh-test -- -test.run $(TEST)

debugtest1gits: testbin-gits
	cd pkg/gits && dlv --log --listen=:2345 --headless=true --api-version=2 exec ../../build/gh-test-gits -- -test.run $(TEST)
